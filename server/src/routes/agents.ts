...
export function agentRoutes(
  ...
  // The 2026.916.x release stores broad legacy roles, so the exact pod seat is
  // fail-closed on both the role and title until the canonical role taxonomy
  // reaches a stable release.
  const autonomousDeliveryPodHireSeats = new Map<string, Set<string>>([
    ["engagement manager", new Set(["pm"])],
    ["spec writer", new Set(["general"])],
    ["tester", new Set(["qa"])],
    ["builder", new Set(["engineer"])],
    ["release manager", new Set(["devops"])],
    ["pod devops", new Set(["devops"])],
    ["architect", new Set(["engineer"])],
    ["designer", new Set(["designer"])],
    ["researcher", new Set(["researcher"])],
    ["engineer", new Set(["engineer"])],
    ["independent verifier", new Set(["qa"])],
  ]);

  function deliveryPodSeat(role: string | null | undefined, title: string | null | undefined) {
    if (!role || !title) return null;
    const normalizedTitle = title.trim().toLowerCase();
    for (const [seat, roles] of autonomousDeliveryPodHireSeats) {
      if ((normalizedTitle === seat || normalizedTitle.endsWith(` ${seat}`)) && roles.has(role)) return seat;
    }
    return null;
  }
  ...
  async function qualifiesForAutonomousDeliveryPodHire(
    ...
    hireInput: { role?: string | null; title?: string | null; reportsTo?: string | null; permissions?: unknown },
  ) {
    if (!actorAgent || actorAgent.role !== "pm" || !actorAgent.title?.toLowerCase().includes("program manager")) return false;
    ...
    const seat = deliveryPodSeat(hireInput.role, hireInput.title);
    ...
    return Boolean(
      ...
      && manager.role === "pm"
      && manager.title?.toLowerCase().endsWith("engagement manager")
